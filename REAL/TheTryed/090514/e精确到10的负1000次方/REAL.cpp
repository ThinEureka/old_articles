	...
	*/
	...
	void IToString( const REAL &R,std::string &s,int &nHeight,int nSpace,char cBase)
	{
		...
		case 'h':case 'H':
			{
				int nBlock = I.Head() - 1;
				REAL::typeUnit nMask = 0xF << REAL::sc_nUnitBit  - 4;
				bool bTopZero = true;
				for( int i = 0; i< nSpace + 3; ++i)
				{
					REAL::typeUnit uE = I.ce(nBlock) & nMask;
					nMask >>= 4;
					if( nMask == 0)
					{
						nMask = 0xF;
		...
		case 'h':case 'H':
			{
				...
				for( int i = 0;i<nSpace+3;++i)
				{
					REAL::typeUnit uE = F.ce(nBlock) & nMask;
					s += REAL::IntToDigit(uE);
					nMask >>= 4;
					if( nMask == 0)
					{
						nMask = 0xF << (REAL::sc_nUnitBit - 4);
						--nBlock;
					}
				}
				int nIndex = s.find_first_not_of("0");
